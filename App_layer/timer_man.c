             ...
             int (*handler)(u_int8_t *buf, size_t data_size, shm_struct *shm, int sem_id))
{
    ...
    tmr_node->buf = buf;
    tmr_node->data_size = data_size;
    tmr_node->shm = shm;
    tmr_node->sem_id = sem_id;
    tmr_node->handler = handler;
    tmr_node->next = NULL;
    /*
    get_tm_stamp(tm_stamp);
    fprintf(timer_fp, "\n%s ADD Timer: [%d:%d]->{%p:%ld:%p:%d} \n", tm_stamp, cnt, interval, tmr_node->buf,\
            tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
    fprintf(stdout, "\n%s ADD Timer: [%d:%d]->{%p:%ld:%p:%d} \n", tm_stamp, cnt, interval, tmr_node->buf,\
            tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
    */
...
int proc_timer()
{
    ...
    if(tm_wheel[cnt].front != NULL){
        ...
        /*
        get_tm_stamp(tm_stamp);
        fprintf(timer_fp, "\n%s PROCESS AND REMOVE Timer: [%d]->{%p:%ld:%p:%d} \n", tm_stamp, cnt, tmr_node->buf,\
                tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
        fprintf(stdout, "\n%s PROCESS AND REMOVE Timer: [%d]->{%p:%ld:%p:%d} \n", tm_stamp, cnt, tmr_node->buf,\
                tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
        */
        rc = tmr_node->handler(tmr_node->buf, tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
        if(rc < 0){
    ...
    for(i = 0; i < TIME_WHEEL_SIZE; i++){
        if(tm_wheel[i].front != NULL){
            ...
            while(tmr_node->next != NULL){
                tmr_node = tmr_node->next;
                get_tm_stamp(tm_stamp);
                fprintf(timer_fp, "\n%s ONLY REMOVE Timer: [%d]->{%p:%ld:%p:%d} \n", tm_stamp, cnt, tmr_node->buf,\
                        tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
                fprintf(stdout, "\n%s ONLY REMOVE Timer: [%d]->{%p:%ld:%p:%d} \n", tm_stamp, cnt, tmr_node->buf,\
                        tmr_node->data_size, tmr_node->shm, tmr_node->sem_id);
